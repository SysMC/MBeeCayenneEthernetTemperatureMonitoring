 ...
 * CONSTANTS
 */
#define DELAY_BEFORE_FRAMES 1000
...
uint8_t getCommandCode(unsigned char*);
 ...
 * @fn      getCheckSum
 *
 * @brief   Подсчет контрольной суммы в соответствии с алгоритмом, используемым в API-фреймах DIGI
 *
 * @param   uint8_t* - указатель на буфер с данными
 *          uint8_t - длина поля данных
 *
  ...
  if (*buffer_p++ == 0x7E)
  {
    ...
    if (i >= sizeof(receivePacket_t) - 1)
    {
      if (getCheckSum(&receivePacket.frameType, receivePacket.frameHeader.lengthLsb) == receivePacket.checkSum)  //Проверяем контрольные суммы принятого пакета.
      {
        ...
        {
          //Serial.write((char*)&receivePacket, sizeof(receivePacket_t)); //Отправляем эхо пакета для диагностики.
